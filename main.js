...
function draw() {
    ...
    if(magentalip_stats == true) {
        image(magenta_lip, magenta_lip_x, magenta_lip_y, 70, 40)
    } else if (redlip_stats = true) {
        image(red_lip, red_lip_x, red_lip_y, 70, 40);
    } else if (pinklip_stats = true) {
        image(pink_lip, pink_lip_x, pink_lip_y, 70, 40);
    }
}
...
function gotPoses(results) {
    ...
    {
        console.log(results);
        magenta_lip_x = results[0].pose.nose.x - 40;
        magenta_lip_y = results[0].pose.nose.y;
        red_lip_x = results[0].pose.nose.x - 40;
        red_lip_y = results[0].pose.nose.y;
        pink_lip_x = results[0].pose.nose.x - 40;
        pink_lip_y = results[0].pose.nose.y;
